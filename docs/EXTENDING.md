...
The key names something in another directory: the rule *reads* it, a `kit/commands/lodestar-*.md` spec or a `kit/templates/hooks/*.py` hook *writes* it. Rename either side alone and the rule silently changes character — a nag with no off switch, or silence about a gap that is still open — so `validate.py` requires the dotted path to exist as a **path** among the writers: the same nesting, in a spec's fenced JSON block or in a hook's assignment into the manifest. Two unrelated sibling keys do not satisfy a two-segment flag, because the engine walks the path node by node and reports "missing" the moment an intermediate is not a dict. Prose about a rule is not scanned; prose does not write the manifest. A new self-silencing rule is therefore not finished until whatever closes its gap records it.
