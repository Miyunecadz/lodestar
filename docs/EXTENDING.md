...
The key names something in another directory: the rule *reads* it, a `kit/commands/lodestar-*.md` spec or a `kit/templates/hooks/*.py` hook *writes* it. Rename either side alone and the rule silently changes character — a nag with no off switch, or silence about a gap that is still open — so `validate.py` requires the dotted path to be one the engine could actually walk to: the same nesting, either in a command spec's manifest fence or in a hook's assignment into the manifest dict.

A spec marks the blocks that write the manifest by opening the fence <code>```json manifest</code>. Its other JSON blocks describe `.claude/settings.json`, `source.json`, or a *repo entry* inside `repos[]`, and a rule keyed on one of those keys would nag forever just as surely as one keyed on a typo — the marker is what tells them apart, since the JSON alone cannot. Within a marked fence, a key under an array is not a path either: `manifest_missing()` walks node by node and reports "missing" the moment an intermediate is not a dict, so `repos[].mapping` is unreachable and two unrelated sibling keys never add up to a two-segment flag. Prose about a rule is not scanned; prose does not write the manifest. A new self-silencing rule is therefore not finished until whatever closes its gap records it — under a marked fence, if a command is what closes it.
