...
|---|---|---|
| `ci.yml` → job `ci` | every PR + push to `main` | shellchecks **every** `.sh` in the repo (discovered with `find`, so a new script cannot be forgotten), validates the catalog + `VERSION`↔`CHANGELOG`, and smoke-tests the guardrail engine, the installed-rule drift checker, the installer, the commit-surface checker, graph coverage, graph freshness, and the permission surface. **Required status check.** |
