...
|---|---|---|
| `ci.yml` → job `ci` | every PR + push to `main` | shellchecks `install.sh` and the shell tests, validates the catalog + `VERSION`↔`CHANGELOG`, and smoke-tests the guardrail engine, the installer, the commit-surface checker, graph coverage, graph freshness, and the permission surface. **Required status check.** |
