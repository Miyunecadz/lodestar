...
|---|---|---|
| `ci.yml` | every PR + push to `main` | shellchecks `install.sh` and the shell tests, validates the catalog + `VERSION`↔`CHANGELOG`, and smoke-tests the guardrail engine, the installer, the commit-surface checker, graph coverage, graph freshness, and the permission surface. **This is the required status check.** |
