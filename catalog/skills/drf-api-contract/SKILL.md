...
The shared REST API surface is the spine every repo depends on. Its single source of truth is **`docs/_shared/api-contract.md`** — read it first and plan the change against it.
...
- **Keep the schema in sync.** Any change to the surface updates `docs/_shared/api-contract.md` and the OpenAPI schema in the same change. A drifted contract is worse than no contract.
