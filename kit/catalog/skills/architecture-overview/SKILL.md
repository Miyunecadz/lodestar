...
3. Per-repo structure: read the repo's `architecture` mode in `.claude/lodestar.manifest.json` first — `graphify` → prefer **querying** `docs/<repo>/architecture/graph.json` (built for querying without re-reading files) over re-reading source, and before treating an empty answer as "no such code" check that repo's `mapping.coverage.filesMissing` — a file the graph does not cover is unmapped, not absent; `markdown` → read `docs/<repo>/architecture/overview.md`; `deferred` → no map exists, read source.
