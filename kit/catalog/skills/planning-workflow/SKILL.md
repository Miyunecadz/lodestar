...
1. Read `docs/_shared/api-contract.md` — the API contract is the **spine**. A section still marked `<!-- TODO: human -->` is unknown, not empty: say so and read the code rather than inferring the contract. Read `docs/repo-map.md` to see which repos the feature touches and how.
