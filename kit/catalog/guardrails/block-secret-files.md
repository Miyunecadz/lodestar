...
**Surface: `both`.** Also enforced for every committer: a staged private key or credential file blocks the commit. Note the agent surface covers Edit/Write only — the engine is registered for `Bash|Edit|Write|MultiEdit` and cannot intercept a `Read`, so "never read" is guidance to the model rather than an enforced stop.
