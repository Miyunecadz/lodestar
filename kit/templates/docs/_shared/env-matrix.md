...
> **How this gets filled:** `/lodestar-onboard` pre-fills what it can ground in the repo (config mechanism, variable *names* from `.env.example`, the dev endpoint) with a cited basis, and leaves `<!-- TODO: human -->` for staging/prod URLs and the secret store. Never put secret *values* here.
