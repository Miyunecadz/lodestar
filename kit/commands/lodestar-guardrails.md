...
- If `emits: rule`: write `.claude/guardrails/<id>.md` with frontmatter `name: <id>`, `enabled: true`, `event`, `pattern`, `severity` (`block`/`warn`), and the message body from the catalog entry — copied verbatim (a `block` message must redirect to the correct alternative, not just deny). **Copy the whole body including the `---` separator and everything below it.** The engine sends only the part above that line when the rule fires; the rationale below stays in the workspace so anyone opening the rule can see why it exists. Stripping it would leave the user with an unexplained rule. Keeping one rule per file in this folder is the whole point: `.claude/` root stays clean.
  - **Copy the catalog entry's `stacks`, context flags, and surface fields too** — `stacks`, `allow_if_untracked`, `only_on_default_branch`, `match`, `allow_paths`, `ignore_case`, `surface`, `permission_rules`, `commit_check`, `commit_severity`, `requires_manifest_missing`. These are what the engine's context layer reads; a rule installed without them silently loses its scoping and fires everywhere. Dropping `requires_manifest_missing` costs a rule its *self-silencing* instead: it is the key that stops a reminder firing once the workspace records the thing it asks for, so a rule installed without it nags forever. Copy the values verbatim, including list syntax (`stacks: [react-native]`, `surface: [agent, commit, permission]`).
