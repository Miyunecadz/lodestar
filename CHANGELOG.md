...
Headings carry the release date. `— Unreleased` marks the top entry while its release PR is open and no tag exists yet. `— not released` marks a version that was written up but never cut — see 0.8.0 / 0.9.0 below. `validate.py` enforces all three against the real git tags.

Sections here are assembled by `.github/scripts/release.py` from `changelog.d/` fragments. Do not edit this file in a feature PR — see [`changelog.d/README.md`](changelog.d/README.md).
